  ...
  return <section className="module-page">
    ...
    <div className="module-panel">
      ...
      <div className="table-wrap"><table className="module-table"><thead><tr><th>Usuário</th><th>E-mail</th><th>Cargo</th><th>Perfil</th><th>Status</th><th>Último acesso</th><th>Ações</th></tr></thead><tbody>
        {filtered.map((r:any,i:number)=>{const linked=new Set((r.usuario_empresas||[]).map((x:any)=>x.empresa_id));return <tr key={r.id}><td><div className="entity-cell"><InitialAvatar name={r.nome||r.email||'Usuário'} index={i}/><b>{r.nome||'Sem nome'}</b></div></td><td>{r.email||'—'}</td><td>{r.cargo||'Não informado'}</td><td><span className="role-chip">{roleLabel[r.perfil]||r.perfil}</span></td><td><span className={`status-chip ${r.ativo?'success':'danger'}`}>{r.ativo?'Ativo':'Inativo'}</span></td><td>{r.ultimo_acesso?new Date(r.ultimo_acesso).toLocaleString('pt-BR',{timeZone:'America/Sao_Paulo'}):'—'}</td><td><details className="row-editor user-editor"><summary aria-label={`Editar ${r.nome||r.email}`}><Pencil size={14}/></summary><form action={alterar}>
