...
async function editar(f: FormData) {"use server"; const s=await createClient(); const {error}=await s.from("candidatos").update({nome:String(f.get("nome")),email:String(f.get("email")||"")||null,telefone:String(f.get("telefone")||"")||null,cargo:String(f.get("cargo")||"")||null,cidade:String(f.get("cidade")||"")||null,estado:String(f.get("estado")||"")||null,linkedin_url:String(f.get("linkedin_url")||"")||null,observacoes:String(f.get("observacoes")||"")||null,situacao:String(f.get("situacao")||"disponivel")}).eq("id",String(f.get("id"))); if(error)throw new Error(error.message); revalidatePath("/candidatos");revalidatePath("/candidaturas");revalidatePath("/dashboard");}
  ...
  const [{ data: rows }, { data: apps }, { data: interviews }] =
    await Promise.all([
      ...
      s.from("entrevistas").select("candidato_id,status,data,duracao_minutos,link,observacoes").order("data",{ascending:false}),
    ]);
    ...
    <section className="module-page">
      ...
      <div className="module-panel">
        <form className="filterbar" method="get">
          ...
          <select name="situacao" defaultValue={params.situacao||""}><option value="">Todos os status</option><option value="disponivel">Disponível</option><option value="empregado">Empregado</option></select><button className="outline-button">Filtrar</button>{(params.q||params.situacao)&&<Link className="outline-button" href="/candidatos">Limpar filtros</Link>}
          ...
          <table className="module-table">
            ...
            <tbody>
              {list.map((r: any, i: number) => {
                ...
                const interview=(interviews||[]).find((x:any)=>x.candidato_id===r.id);
                return (
                  <tr key={r.id}>
                    <td>
                    ...
                    <td>
                      <div className="row-actions">
                        <ActionModal kind="view" title={r.nome}><div><h3>{r.nome}</h3><p><strong>E-mail:</strong> {r.email||"—"}</p><p><strong>Telefone:</strong> {r.telefone||"—"}</p><p><strong>Cargo:</strong> {r.cargo||"—"}</p><p><strong>Cidade/UF:</strong> {[r.cidade,r.estado].filter(Boolean).join("/")||"—"}</p><p><strong>Situação:</strong> {r.situacao==="empregado"?"Empregado":"Disponível"}</p><p><strong>LinkedIn:</strong> {r.linkedin_url||"—"}</p><p><strong>Processo atual:</strong> {app?.vagas?.cargo||"—"} · {app?.vagas?.empresas?.nome||"—"}</p><p><strong>Etapa:</strong> {app?.status||"—"}</p><p><strong>Avaliação:</strong> {app?.avaliacoes?.[0]?.nota_tecnica?String(app.avaliacoes[0].nota_tecnica)+"/5":"—"}</p><p><strong>Parecer:</strong> {app?.avaliacoes?.[0]?.parecer||"—"}</p><p><strong>Entrevista:</strong> {interview?.data?new Date(interview.data).toLocaleString("pt-BR"):"—"}{interview?.duracao_minutos?` · ${interview.duracao_minutos} min`:""}{interview?.status?` · ${interview.status}`:""}</p><p><strong>Observações da entrevista:</strong> {interview?.observacoes||"—"}</p><p><strong>Origem do processo:</strong> {app?.origem||"—"}</p><p><strong>Inscrição:</strong> {app?.inscrito_em?new Date(app.inscrito_em).toLocaleDateString("pt-BR"):"—"}</p><p><strong>Observações:</strong> {r.observacoes||"—"}</p><ResumeActions candidateId={r.id} path={r.curriculo_path}/></div></ActionModal>
