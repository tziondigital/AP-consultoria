...
type Kind='view'|'edit';
export function ActionModal({kind,title,children,trigger}:{kind:Kind;title:string;children:ReactNode;trigger?:ReactNode}){
 ...
 return <>{trigger?<button type="button" className="text-button" onClick={()=>setOpen(true)}>{trigger}</button>:<button type="button" className="icon-button" title={kind==='edit'?'Editar':'Visualizar'} aria-label={(kind==='edit'?'Editar ':'Visualizar ')+title} onClick={()=>setOpen(true)}><Icon size={14}/></button>
