...
export function ActionModal({kind,title,children,triggerLabel}:{kind:Kind;title:string;children:ReactNode;triggerLabel?:string}){
 ...
 return <><button ref={triggerRef} type="button" className={triggerLabel?'text-trigger':'icon-button'} title={kind==='edit'?'Editar':'Visualizar'} aria-label={(kind==='edit'?'Editar ':'Visualizar ')+title} onClick={()=>setOpen(true)}>{triggerLabel?<span>{triggerLabel}</span>:<Icon size={14}/>}</button>
