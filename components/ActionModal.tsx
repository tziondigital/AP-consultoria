...
export function ActionModal({kind,title,children,triggerLabel}:{kind:Kind;title:string;children:ReactNode;triggerLabel?:string}){
 ...
 return <><button ref={triggerRef} type="button" className={triggerLabel?'text-trigger action-trigger-text':`icon-button action-trigger action-trigger-${kind}`} title={kind==='edit'?'Editar':'Visualizar'} aria-label={(kind==='edit'?'Editar ':'Visualizar ')+title} onClick={()=>setOpen(true)}>{triggerLabel?<span>{triggerLabel}</span>:<Icon size={14}/>}</button>
 {open&&<div className="record-modal-backdrop" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)setOpen(false)}}>
  <section ref={panelRef} className="record-modal" role="dialog" aria-modal="true" aria-labelledby={titleId}>
   <header className="record-modal-head"><div><b id={titleId}>{kind==='edit'?'Editar':'Visualizar'} — {title}</b><span>{kind==='edit'?'Atualize as informações e salve as alterações.':'Informações completas do registro.'}</span></div><button type="button" className="icon-button modal-close-button" onClick={()=>setOpen(false)} aria-label="Fechar"><X size={18}/></button></header>
