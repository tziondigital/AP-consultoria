...
export function ActionModal({kind,title,children,triggerLabel}:{kind:Kind;title:string;children:ReactNode;triggerLabel?:string}){
 ...
 return <>{triggerLabel?<button type="button" className="text-button" onClick={()=>setOpen(true)}>{triggerLabel}</button>:<button type="button" className="icon-button" title={kind==='edit'?'Editar':'Visualizar'} aria-label={(kind==='edit'?'Editar ':'Visualizar ')+title} onClick={()=>setOpen(true)}><Icon size={14}/></button>
